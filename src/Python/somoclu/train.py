...
try:
    ...
    print(
        "Warning: the binary library cannot be imported. You cannot train "
        "maps, but you can load and analyze ones that you have already saved."
    )
    if sys.platform.startswith("win"):
        print(
            "If you installed Somoclu with pip on Windows, this typically "
            "means missing DLLs. Please refer to the documentation."
        )
    elif sys.platform.startswith("darwin"):
        print(
            "If you installed Somoclu with pip on macOS, this typically "
        ...
        print(
            "The problem occurs because either compilation failed when you "
...
class Somoclu(object):
    ...
    def __init__(
        ...
        if initialcodebook is not None and initialization is not None:
            raise Exception(
                "An initial codebook is given but initilization" " is also requested"
            )
        self.bmus = None
    ...
    def load_bmus(self, filename):
        ...
        if self.n_vectors != 0 and len(self.bmus) != self.n_vectors:
            raise Exception(
                "The number of best matching units does not match "
        ...
        self.bmus[:, 1] = tmp
        if (
            max(self.bmus[:, 0]) > self._n_columns - 1
            or max(self.bmus[:, 1]) > self._n_rows - 1
        ):
            raise Exception(
                "The dimensions of the best matching units do not "
    ...
    def load_umatrix(self, filename):
        ...
        if self.umatrix.shape != (self._n_rows, self._n_columns):
            raise Exception(
                "The dimensions of the U-matrix do not " "match that of the map"
            )
    ...
    def load_codebook(self, filename):
        ...
        if self.codebook.shape != (self._n_rows * self._n_columns, self.n_dim):
            raise Exception(
                "The dimensions of the codebook do not " "match that of the map"
            )
    ...
    def train(
        ...
        wrap_train(
            ...
            self._map_type,
            self._grid_type,
            self._compact_support,
            self._neighborhood == "gaussian",
            self._std_coeff,
            self._verbose,
            self.codebook,
            self.bmus,
    ...
    def view_component_planes(
        ...
        if self.codebook is None:
            raise Exception(
                "The codebook is not available. Either train a map"
                " or load a codebook from a file"
            )
        if dimensions is None:
            dimensions = range(self.n_dim)
        for i in dimensions:
            plt = self._view_matrix(
                self.codebook[:, :, i],
                figsize,
                colormap,
                colorbar,
    ...
    def view_umatrix(
        ...
        if self.umatrix is None:
            raise Exception(
                "The U-matrix is not available. Either train a map"
        ...
        if data_vector is not None:
            ...
            matrix = cdist(
                self.codebook.reshape(
                    (
                        self.codebook.shape[0] * self.codebook.shape[1],
            ...
            matrix = activation_map[data_index].reshape(
                (self.codebook.shape[0], self.codebook.shape[1])
            )
        return self._view_matrix(
            matrix,
    ...
    def _view_matrix(
        ...
        if self._grid_type == "hexagonal":
            offsets = _hexplot(
                matrix[zoom[0][0] : zoom[0][1], zoom[1][0] : zoom[1][1]], fig, colormap
            )
            ...
            plt.imshow(
                matrix[zoom[0][0] : zoom[0][1], zoom[1][0] : zoom[1][1]],
                aspect="auto",
        ...
        if labels is not None:
            for label, col, row in zip(
                self._filter_array(labels, zoom), bmu_coords[:, 0], bmu_coords[:, 1]
            ):
    ...
    def _check_parameters(self):
        ...
        if self._map_type != "planar" and self._map_type != "toroid":
            raise Exception("Invalid parameter for _map_type: " + self._map_type)
        if self._grid_type != "rectangular" and self._grid_type != "hexagonal":
            raise Exception("Invalid parameter for _grid_type: " + self._grid_type)
        if self._neighborhood != "gaussian" and self._neighborhood != "bubble":
            raise Exception("Invalid parameter for neighborhood: " + self._neighborhood)
        if self._kernel_type != 0 and self._kernel_type != 1:
            raise Exception("Invalid parameter for kernelTye: " + self._kernel_type)
        if self._verbose < 0 and self._verbose > 2:
            raise Exception("Invalid parameter for verbose: " + self._kernel_type)
    ...
    def _pca_init(self):
        ...
        for j in range(self._n_columns * self._n_rows):
            for i in range(eigvec.shape[0]):
                self.codebook[j, :] = self.codebook[j, :] + coord[j, i] * eigvec[i, :]
    ...
    def _init_codebook(self):
        ...
        if self.codebook is None:
            ...
            if self.codebook.dtype != np.float32:
                print(
                    "Warning: initialcodebook was not float32. A 32-bit "
                    "copy was made"
                )
    ...
    def get_bmus(self, activation_map):
        ...
        Y, X = np.unravel_index(
            activation_map.argmin(axis=1), (self._n_rows, self._n_columns)
        )
        return np.vstack((X, Y)).T

    def view_similarity_matrix(
        self, data=None, labels=None, figsize=None, filename=None
    ):
        ...
        if not have_heatmap:
            raise Exception(
                "Import dependencies missing for viewing "
